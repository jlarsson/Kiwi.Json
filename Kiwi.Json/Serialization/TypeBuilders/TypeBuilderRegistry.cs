...
namespace Kiwi.Json.Serialization.TypeBuilders
{
    public class TypeBuilderRegistry : ITypeBuilderRegistry
    {
        ...
        {
            Func<ITypeBuilderRegistry, ITypeBuilder> factory = default(Func<ITypeBuilderRegistry, ITypeBuilder>);
        ...
        private Func<ITypeBuilderRegistry, ITypeBuilder> TryCreateTypeBuilderForClass(Type type)
        {
            if (type.IsClass)
            {
                ...
                if (constructor == null)
                {
                    return _ => new InvalidClassBuilder(type);
                }
        ...
        private Func<ITypeBuilderRegistry, ITypeBuilder> TryCreateTypeBuilderForEnumerable(Type type)
        {
            ...
            Type interfaceType = (from @interface in new[] {type}.Concat(type.GetInterfaces())
                                  where
            ...
            Type listType = type.IsInterface
